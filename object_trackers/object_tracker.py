...
from system.utilities import device, CocoDetectedObject
...
class ObjectTracker:
    ...
    @staticmethod
    def _draw_detecteds(img, di: CocoDetectedObject, box) -> List[CocoDetectedObject]:
    ...
    def _draw_boxes_not_mine(self, detected_boxes, img, bbox, pred_score, di: CocoDetectedObject, identities=None,
                             offset=(0, 0)):
        ...
        for i, box in enumerate(bbox):
            ...
            di.track_id = id
            print('id: ', id)
            label = f'{di.get_pred_cls()} {pred_score:.2f} ({id:d})'
    ...
    def detect(self, img: np.array, detected_boxes) -> List[CocoDetectedObject]:
        dis:List[CocoDetectedObject] = []
        ...
        for box in detected_boxes:
            *xyxy, conf, cls = box
            ...
            if len(outputs) > 0:
                ...
                di = CocoDetectedObject()
                di.img = img
                di.pred_score = conf
                di.pred_cls_indx = int(cls.item())
                dis.append(di)
